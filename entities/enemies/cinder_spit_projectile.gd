# cinder_spit_projectile.gd
# The blob of molten spit a ranged monster (the Cinder Spitter) launches at the
# player. It's an Area3D that flies in a straight line, carrying a single one_shot
# HitBox (target_team = PLAYER). The moment that HitBox lands on the player's HurtBox
# it deals damage and the whole projectile frees itself; if it never hits anything it
# self-destructs after `max_lifetime` so stray shots don't pile up.
#
# The enemy doesn't poke our internals — it just calls launch(dir, amount, type,
# source) right after instancing us (duck-typed in enemy.gd._spawn_projectile). We
...
#
# We move ourselves manually each physics frame (no RigidBody): straight-line travel
# is all a spit needs, and it keeps the HitBox overlap simple and predictable.

extends Area3D

## Travel speed in m/s. Tune per projectile type in the Inspector.
@export var speed: float = 12.0

## Seconds before an un-hit projectile gives up and frees itself.
...
# the spit only DETONATES on geometry, never on a living body (the HitBox handles those hits).
...
# Called by the spawning enemy right after instancing. Stores the heading and builds
# the one_shot HitBox payload that hits the player. `knockback` is optional (defaults
# to 0) so older 4-arg callers keep working unchanged.
func launch(direction: Vector3, amount: float, damage_type: DamageInfo.DamageType, source: Node, knockback: float = 0.0) -> void:
	_direction = direction.normalized()
	_source = source

	# Build the damage volume: one_shot so it frees on first contact, target PLAYER so
	# it can only hurt the player (never its own kind), source = the monster for credit.
	_hit_box = HitBox.new()
	...
	# When the HitBox is freed (it free's itself on its first landed hit), tear down the
	# whole projectile so the visual blob vanishes with the impact.
...
func _physics_process(delta: float) -> void:
	# Straight-line flight; gravity-free so spit arcs read as a "magic" projectile and
	# aim stays predictable. Sweep this frame's segment for solid world first so the spit
	# splatters on walls/floors instead of sailing through them.
